...
export const asyncGetConfigData = async (cb: AsyncCallback, data?:Record<string, unknown>) => {
  const res = await YSAxios.post(api.configList,data).catch((e) => {
...
export const asyncPostDemo = async (data: IConfig, cb: AsyncCallback) => {
...
export const asyncPutConfig = async (data: IConfig, cb: AsyncCallback) => {
  const res = await YSAxios.put(`${api.config}`, data).catch((e) => {
    message.error(e.message);
    return e;
  });
...
export const asyncDelUser = async (data: IConfig, cb: AsyncCallback) => {
...
export const asyncResetPassword = async (data: IConfig, cb: AsyncCallback) => {
  const res = await YSAxios.post(`${api.user}/resetPassword/${data.id}`).catch((e) => {
    message.error(e.message);
    return e;
  });
  callback(res, cb);
};
export const asyncLockUser = async (data: IConfig, cb: AsyncCallback) => {
  const res = await YSAxios.post(`${api.user}/lock/${data.id}`).catch((e) => {
    message.error(e.message);
    return e;
  });
  callback(res, cb);
